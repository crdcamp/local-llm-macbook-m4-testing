...
ax3.set_xticklabels(['Hard', 'Medium', 'Easy'])
ax3.set_title('Prompt Difficulty vs. TPS')

plt.show()

1# %% Now we should calculate ratios or something between these....
...
pivot_tables = {
    ...
    "tps": pd.pivot(benchmarks, index='prompt', columns='model', values='tokens_per_second')
}

for key, value in pivot_tables.items():
    pivot_tables[key] = pivot_tables[key].drop(['8B_qwen_3', '0.5B_ruvltra'], axis=1) # These are bad data imo
...
# Prompt Difficulty vs. Tokens/Second
# (This graph might be useless)
pivot_tables['tps'].plot(kind='bar', ax=ax3)
ax3.set_title('Prompt Difficulty vs. Tokens/Second')
ax3.set_xlabel('Prompt Difficulty')
ax3.set_ylabel('Tokens/Second')
ax3.set_xticklabels(['Hard', 'Medium', 'Easy'])
...
"""
Welp.... now I guess it's time to make a few shell scripts for llama bench to get some actual metrics
instead of doing all of this by hand.
"""
